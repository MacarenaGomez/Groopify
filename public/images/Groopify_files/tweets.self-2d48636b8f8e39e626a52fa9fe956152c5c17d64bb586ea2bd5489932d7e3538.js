(function() { 
  ...
  function getTweets(response){
   ...
   if (response.others.length > 0 ){
    ...
    var $tweetsContainer = $('[data-expert=' + response.profile.screen_name +']');
       ...
       tweetParts.push('  <div class="row pepe">');
    ...
    $('[data-hook="tweets"]').after('<h1 class="most-interesting">Others you may be interested in:</h1>');
  ...
  function getTweetsOther(response){
    ...
    var $tweets = $('[data-hook=tweets]')
    
    var htmlParts  = ['<div data-hook="tweets" class="col-md-12 tweets">',
                      '  <div class="row">',
                      '    <div class="span5">']
    if (response.tweets.length > 0){
      htmlParts.push('      <h1 class="most-interesting">More interesting tweets</h1>');
      htmlParts.push('      <table class="table">');
      htmlParts.push('        <tr>');
      ...
      htmlParts.push('<h1 class="most-interesting"No tengo tweets?!</h1>');
      htmlParts.push('  </div>'),
      htmlParts.push('</div>');
      htmlParts.push('</div>');
    }
    $expertContainer.after(htmlParts.join('\n'));
    $('#loading').addClass("hidden");
  }
